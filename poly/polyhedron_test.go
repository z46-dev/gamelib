...
func TestPolyhedronConstructionTransformAndQueries(t *testing.T) {
	...
	assert.InDelta(t, 4, shape.AABB.X1, gmath.EPSILON)
	assert.InDelta(t, -4, shape.AABB.Y1, gmath.EPSILON)
	assert.InDelta(t, -2, shape.AABB.Z1, gmath.EPSILON)
	assert.InDelta(t, 6, shape.AABB.X2, gmath.EPSILON)
	assert.InDelta(t, 0, shape.AABB.Y2, gmath.EPSILON)
	assert.InDelta(t, 4, shape.AABB.Z2, gmath.EPSILON)
}
...
func TestPolyhedronMeasurementsAndSurfaceQueries(t *testing.T) {
	...
	assert.InDelta(t, 24, shape.SurfaceArea(), gmath.EPSILON)
	assert.InDelta(t, 8, shape.Volume(), gmath.EPSILON)
	assert.NotZero(t, shape.SignedVolume())
	centroid = shape.Centroid()
	assert.InDelta(t, 0, centroid.X, gmath.EPSILON)
	assert.InDelta(t, 0, centroid.Y, gmath.EPSILON)
	assert.InDelta(t, 0, centroid.Z, gmath.EPSILON)

	normal, ok = shape.FaceNormal(0)
	assert.True(t, ok)
	assert.InDelta(t, -1, normal.Z, gmath.EPSILON)
	_, ok = shape.FaceNormal(-1)
	assert.False(t, ok)

	closest = shape.ClosestPoint(vector.NewVec3(3.0, 0.25, -0.5))
	assert.InDelta(t, 1, closest.X, gmath.EPSILON)
	assert.InDelta(t, 0.25, closest.Y, gmath.EPSILON)
	assert.InDelta(t, -0.5, closest.Z, gmath.EPSILON)
...
func TestPolyhedronRayLineAndSphereQueries(t *testing.T) {
	...
	assert.True(t, hit)
	assert.InDelta(t, 2, distance, gmath.EPSILON, "distance must not depend on direction magnitude")
	assert.InDelta(t, -1, point.X, gmath.EPSILON)
...
func TestPolyhedronBVHAndConvexSurfaceParts(t *testing.T) {
	...
	assert.InDelta(t, -1, parts[0].Normal.Z, gmath.EPSILON)
	...
	assert.InDelta(t, 9, shape.ConvexParts()[0].AABB.X1, gmath.EPSILON, "transforms must rebuild cached face bounds")
}
...
func TestPolyhedronIntersectionAndContactManifold(t *testing.T) {
	...
	var reused []poly.PolyhedronContact[float64] = make([]poly.PolyhedronContact[float64], 0, 32)
	manifold = poly.GetPolyhedronContactManifoldInto(first, second, reused)
	assert.NotEmpty(t, manifold.Contacts)
	assert.Equal(t, cap(reused), cap(manifold.Contacts))
